...
public class EndermanClass extends PlayerClassBase {
		...
		if (!fsmash) {
			if (smash) {
				if (player.isSneaking()) {
					...
					SendPacketToAll(new PacketPlayOutWorldEvent(2003,
							new BlockPosition(l.getBlockX(), l.getBlockY() + 1, l.getBlockZ()), 0, false));
	...
	public void PlayerSpawn() {
		...
		i.setChestplate(Colorizer.setLeatherArmorColor(new ItemStack(Material.LEATHER_CHESTPLATE), 37, 6, 39));

		ItemStack legs = Colorizer.setLeatherArmorColor(new ItemStack(Material.LEATHER_LEGGINGS), 37, 6, 39);
		legs.addUnsafeEnchantment(Enchantment.PROTECTION_ENVIRONMENTAL, 2);
		i.setLeggings(legs);

		ItemStack boot = Colorizer.setLeatherArmorColor(new ItemStack(Material.LEATHER_BOOTS), 37, 6, 39);
